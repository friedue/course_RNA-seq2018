...
The scattering of the points for fewer than 32 reads indicates that the standard 
deviation of the fold changes may depend on the mean: the lower the mean read counts
per gene, the higher the standard deviation.
...
Let's save our workspace at this point.
...
DESeq.rlog <- rlog(DESeq.ds, blind = TRUE) # set blind = FALSE if the conditions 
...
pca <- prcomp(t(assay(DESeq.rlog)[select, ]))
head(pca$x)
```
